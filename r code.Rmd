...
Anova_diameteraverages <- aov(Group ~ Average_Diameter.um., data = averagediameter)
summary(Anova_diameteraverages)
```

# T Tests for all Data
In this section t-tests were ran for all of the fringes together, separately and over the averages. These are single sample t tests covering each of the fringes.

```{r}
t.test(diffractiondata$Diameter.um.)
t.test(Firstfringediameterum)
t.test(Secondfringediameterum)
t.test(Thirdfringediameterum)
t.test(Fourthfringediamterum)
t.test(averagediameter$Average_Diameter.um.)
```
